...
#     #                 sent_emails.append({'email': recipient_email, 'status': 'Sent'})
#     #                 print(f'email : {recipient_email} sent success')
#     #
#     #         server.quit()
#     #         sent_df = pd.DataFrame(sent_emails)
#     #         sent_df.to_excel('sent_emails.xlsx', index=False)
...
#     #         sent_df = pd.DataFrame(sent_emails)
#     #         sent_df.to_excel('sent_emails.xlsx', index=False)
#     #         messagebox.showerror("Error", "Some Emails Faild to send")
#
#     def send_emails(self):
#         sent_emails = []
#         max_retries = 3  # Number of times to retry reconnecting
...
#
#                 for index, row in df.iterrows():
#                     if email_counter >= 50:  # Check if the limit is reached
#                         email_counter = 0
#                         server.quit()
...
#                     server.sendmail(email, recipient_email, text)
#                     sent_emails.append({'email': recipient_email, 'status': 'Sent'})
#                     print(f'email : {recipient_email} sent successfully')
#                     email_counter += 1
#
#                 server.quit()
#                 sent_df = pd.DataFrame(sent_emails)
...
class EmailSenderApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        label_email = ttk.Label(self, text="Choose Email Account:", font=("Segoe UI", 16, "bold"))
        self.canvas_bg.create_window(20, 560, anchor='w', window=label_email)

        email_combobox = ttk.Combobox(self, textvariable=self.selected_email, state='readonly',
                                      font=("Segoe UI", 12))
        email_combobox['values'] = list(self.email_credentials.keys())
        email_combobox.current(0)
        self.canvas_bg.create_window(150, 560, anchor='w', window=email_combobox)
